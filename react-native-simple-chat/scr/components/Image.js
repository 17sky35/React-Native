...
const Image = ({url, imageStyle ,showButton, rounded, onChangeImage}) => {
    useEffect(()=>{
       (async () => {
            try {
                if(Platform.OS === "ios"){
    ...
    const _handleEditButton = async () => {
        try {
            const result = await ImagePicker.launchImageLibraryAsync({
                mediaTypes:ImagePicker.MediaTypeOptions.Image,//이미지만 선택할수있음
                allowsEditing:true,//간단한 편집(자르기) 옵션이 제공된다.
                aspect:[1,1],//사용자가 이미지를 편집할 떄 1:1비율로 자를 수 있도록 한다.
